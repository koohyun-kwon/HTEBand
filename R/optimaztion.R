...
#' Optimal weights (time checking purpose)
#'
#' Solve for optimal weigths by the quantile matching method
#'
#' @param c value of c
...
#' }
#' @export
opt_w_2 <- function(c, method, C.vec, y, x, d, eval, T.grad.mat, level,
                  deg, kern, loo, M, seed = NULL, useloop = TRUE,
  ...
  eq <- function(c){
    ...
    if(method %in% c("reg.Hol", "reg.Lip", "TE.Lip", "TE.Lip.eqbw")){
      ...
      print(paste("time took in weight calculation", time.2 - time.1))
      ...
      print(paste("time took in quantile simulation", time.2 - time.1))
    }
